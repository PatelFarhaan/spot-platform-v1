...
from project.credentials.json_schema_validation.create_github_credentials_schema import \
    validate_create_github_credentials_schema
from project.models import Githubcredentials
...
@credentials_blueprint.route('/create-credentials/github', methods=['POST'])
@jwt_required()
@check_if_user_is_logged_in()
def create_github_credentials():
    body = request.get_json()
    validate_create_github_credentials_schema(body)

    body["user"] = current_user
    body["company"] = current_user.company

    try:
        Githubcredentials(**body).save()
